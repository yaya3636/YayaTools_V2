...
function graph.create(V, directed)
    ...
    g.vertexList = dofile(currentDirectory .. '\\YAYA\\Module\\Graph\\data\\list.lua').create()
    g.adjList = {}
    for v = 0,V-1 do
        g.vertexList:add(v)
        g.adjList[v] = dofile(currentDirectory .. '\\YAYA\\Module\\Graph\\data\\list.lua').create()
    end
    g.directed = directed
...
function graph:addVertexIfNotExists(v)
    if self.vertexList:contains(v) then
        ...
        self.adjList[v] = dofile(currentDirectory .. '\\YAYA\\Module\\Graph\\data\\list.lua').create()
        return true
    end
end
...
function graph:edges()
    local list = dofile(currentDirectory .. '\\YAYA\\Module\\Graph\\data\\list.lua').create()
