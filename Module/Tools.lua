...
function Tools:init(params)
    params = params or {}
    self.colorPrint = params.colorPrint or {}
    self.class = Class
    self:InitCellsArray()
end
...
function Tools.monsters:init(params)
    ...
    self.tools = Tools()
    self.json = Tools.json()
    self.ModifiedMonsters = self.json:decode(self.tools:ReadFile(global:getCurrentDirectory() .. "\\YayaTools\\Data\\Monsters\\monsters.json"))
end
...
function Tools.zone:init(params)
    ...
    self.tools = Tools()
    self.json = Tools.json()
    self.bigDataSubAreas = self.json:decode(self.tools:ReadFile(global:getCurrentDirectory() .. "\\YayaTools\\Data\\SubArea\\SubAreas.json"))
end
...
function Tools.api:init(params)
    ...
    self.tools = Tools()
    self.json = Tools.json()
    self.localAPI.localPort = self.json:decode(self.tools:ReadFile(global:getCurrentDirectory() .. "\\YayaTools\\LocalAPI\\ConfigAPI.json")).port
